...
from nmt_core.nmt_data_utils import build_vocab_from_iterator, get_tokenizer

from nmt_core.dataset_zh2en import NEWSCOM # News Commentary dataset
...
def should_enable_tqdm():
    # Keep progress bars in interactive terminals and notebooks, but disable
    # them when output is redirected to a file to avoid one line per refresh.
    if "ipykernel" in sys.modules:
        return True
    stderr = getattr(sys, "stderr", None)
    return bool(stderr is not None and hasattr(stderr, "isatty") and stderr.isatty())
...
from nmt_core.transformer import Transformer # 从transformer.py中导入Transformer类
import math
...
def translate(model: torch.nn.Module, src_sentence: str, temperature: float = 0.0):
    ...
    src_mask = torch.zeros((num_tokens, num_tokens), device=DEVICE, dtype=torch.bool)
    if temperature == 0:
...
def training_state_path(task_name, base_dir=None):
    if base_dir is None:
        base_dir = Path.cwd()
    return Path(base_dir) / "checkpoints" / f"{task_name}_training.pt"


def save_training_state(model, optimizer, task_name, model_config, epoch, training_log, base_dir=None):
    path = training_state_path(task_name, base_dir)
...
def load_training_state_if_exists(model, optimizer, task_name, device, model_config, base_dir=None):
    path = training_state_path(task_name, base_dir)
    if not path.is_file():
        return 1, []
    ...
    if not isinstance(checkpoint, dict) or "model_config" not in checkpoint:
        raise RuntimeError(
            f"Training state {path} does not contain model_config. "
            "Please remove it and restart training."
        )
    ...
    if mismatches:
        ...
        raise RuntimeError(f"Training state hyperparameters do not match current model: {details}")
    ...
    print(f"Resuming zh2en training from epoch {start_epoch}")
